...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/09/27 17:01:06 by naanapa           #+#    #+#             */
/*   Updated: 2024/10/16 14:05:02 by naanapa          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	env_organize(char *key, char *value, t_data *data)
{
	...
	if (env && value)
	{
		...
		add_to_memtrack(env->value);
	}
	else if (!env && (value || key))
	{
		env = memtrack_alloc(sizeof(t_env));
		env->key = key;
		env->value = value;
		env->next = data->env;
		data->env = env;
